...
| `targetWords` | `6` | 提示词中非 CJK 标题的目标词数；实际硬上限由显示宽度决定。 |
| `targetCjkCharacters` | `10` | 标题宽度硬上限（CJK 字数；2 个 ASCII/半角字符算 1 个 CJK）。 |
...
- 总结子代理有固定 8192 token 的输出预算，摘要本身限制在 1000 字符以内；折叠仍失败时
  只推进游标、不重命名，避免同一批事件被反复重喂。
